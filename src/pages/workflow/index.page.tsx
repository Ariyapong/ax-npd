...
const WorkflowDiagram = ({ workflow }: { workflow: Workflow }) => {
  ...
  const [rfInstance, setRfInstance] =
    useState<ReactFlowInstance<CustomNodeData, CustomEdgeData>>();
  ...
  const { selectedNode, selectedEdge } = useSelector(
      ...
      if (source) {
        ...
        if (targetIsPane) {
          ...
          newNode = {
            ...
            position: project({
              x: (event as unknown as MouseEvent).clientX - left - 75,
              y: (event as unknown as MouseEvent).clientY - top,
            }),
            data: {
              label: `New ${id}`,
              precedents: [source],
  ...
  const onChange: React.FormEventHandler<HTMLDivElement> | undefined =
    useCallback(() => {
      // console.log('Changed')
    }, []);

  const onNodeClick: NodeMouseHandler = useCallback(
    (_event, node: Node<CustomNodeData>) => {
      save();
  ...
  useEffect(() => {
    ...
    const icx = document.querySelector(
      ...
      >
        <Panel position="top-right">
          ...
          <div className="updatenode__controls">
            <label>
              <strong>Node Type {selectedNode?.data.type}</strong>
            </label>
            <input
              value={selectedNode?.data.type ?? ""}
              onChange={(evt) =>
                dispatch(setSelectedNodeType(evt.target.value as any))
              }
            />
            <label>
            ...
            <input
              ...
              onChange={(evt) =>
                dispatch(setSelectedNodeLabel(evt.target.value))
              }
            />
            <label>
            ...
            <input
              ...
              onChange={(evt) =>
                dispatch(setSelectedEdgeLabel(evt.target.value))
              }
            />
          </div>
        </Panel>
...
async function onBeforeRender(pageContext: PageContextBuiltIn) {
    ...
    console.log("trigger onBeforeRender", pageContext)
